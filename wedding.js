...
function startOpening(){clearTimers();mainMusicAttempted=false;audio.pause();if(reduced.matches){finishOpening();return}window.scrollTo({top:0,behavior:'instant'});opening.hidden=false;opening.removeAttribute('aria-hidden');opening.className='opening';invitation.inert=true;document.body.classList.remove('arrived');document.body.classList.add('intro-running');later(()=>opening.classList.add('zooming'),210);later(()=>opening.classList.add('unsealing'),648);later(()=>opening.classList.add('unfolding'),816);later(()=>opening.classList.add('card-rising'),2040);later(()=>opening.classList.add('card-fading'),7040);later(()=>{opening.classList.add('dissolving');enterMain()},7440);later(()=>finishOpening(),9240);}
function advanceOpening(){if(opening.hidden||opening.classList.contains('dissolving'))return;if(!opening.classList.contains('unfolding')){opening.classList.add('unsealing','unfolding');return}if(!opening.classList.contains('card-rising')){opening.classList.add('card-rising');return}clearTimers();opening.classList.add('card-fading','dissolving');enterMain();later(()=>finishOpening(),1100);}
